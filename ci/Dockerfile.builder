...
# points_hash
RUN git clone https://github.com/The-Neoxa-Endeavor/points_hash
RUN cd points_hash && python3 setup.py install
...
RUN mkdir /points-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /points-src && \
