...
static void SignTransaction(CMutableTransaction& tx, const CKey& coinbaseKey)
{
    ...
    for (size_t i = 0; i < tx.vin.size(); i++) {
        ...
        std::cout << hashBlock.ToString()<<std::endl;
...
static bool CheckTransactionSignature(const CMutableTransaction& tx)
{
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (!VerifyScript(txin.scriptSig, txFrom->vout[txin.prevout.n].scriptPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, MutableTransactionSignatureChecker(&tx, i, amount))) {
            return false;
        }
    }
    return true;
...
BOOST_FIXTURE_TEST_CASE(dip3_protx, TestChainDIP3Setup)
{
    ...
    int DIP0003EnforcementHeightBackup = Params().GetConsensus().DIP0003Height;
