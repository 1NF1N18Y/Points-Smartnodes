...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for PTS
    ...
    if (p_rewardSnapshot.strDistributionAsset != "PTS") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is PTS with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using PTS differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "PTS") {
        // Check amount
