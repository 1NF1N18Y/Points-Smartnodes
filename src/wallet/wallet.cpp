    ...
    SmartnodeCollaterals collaterals = Params().GetConsensus().nCollaterals;
    {
        ...
        for (auto pcoin : GetSpendableTXs()) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetPTS) {
                    if (fPTSLimitHit) // We hit our limit
                        continue;
                    // We only want PTS OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** PTS START */
                    // Check to see if you need to make an asset data outpoint OP_PTS_ASSET data
                ...
                if (AreAssetsDeployed()) {
                    for (const auto &asset : setAssets){
                        CTxIn txin = CTxIn(asset.outpoint,CScript(),
                                              CTxIn::SEQUENCE_FINAL - 1);
                        vecTxDSInTmp.push_back(CTxDSIn(txin, asset.txout.scriptPubKey));
                        txNew.vin.push_back(txin);
                        }
                }
                /** PTS END */
