...
QString WalletModel::getMnemonic() const
{
    ...
    if (!wallet->GetHDChain(hdChainCurrent))
        return tr("Error: This wallet is not a HD wallet.");

    if (!wallet->GetDecryptedHDChain(hdChainCurrent))
        return tr("Error: Cannot decrypt HD seed.");
    ...
    QString myWords;
    myWords.append(QString::fromStdString(ssMnemonic.c_str()));
    myWords.append("\n");
    ...
    myPass.append(QString::fromStdString(ssMnemonicPassphrase.c_str()));
