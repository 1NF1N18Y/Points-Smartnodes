...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_PTS_ASSET              : return "OP_PTS_ASSET";
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_PTS_ASSET) { // OP_PTS_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == PTS_N) { // Check to see if PTS starts at 27 ( this->size() < 105)
                if ((*this)[28] == PTS_E)
                    if ((*this)[29] == PTS_X)
                        index = 30;
            } else {
                if ((*this)[28] == PTS_N) // Check to see if PTS starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == PTS_E)
                        if ((*this)[30] == PTS_X)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == PTS_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == PTS_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == PTS_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == PTS_N) {
...
{
    // 1 OP_PTS_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_PTS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_PTS_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_PTS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_PTS_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
