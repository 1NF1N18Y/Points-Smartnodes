...
PACKAGE_VERSION=$($SRC_PATH/src/pointsd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/pointsd $DEB_BIN
cp $SRC_PATH/src/points-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/points-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/pointsd.1 $DEB_MAN
cp $SRC_DOC/man/points-cli.1 $DEB_MAN
cp $SRC_DOC/man/points-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/pointsd.bash-completion $DEB_CMP/pointsd
cp $SRC_PATH/contrib/points-cli.bash-completion $DEB_CMP/points-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/pointsd.1
gzip --best -n $DEB_MAN/points-cli.1
gzip --best -n $DEB_MAN/points-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/pointsd $DEB_BIN/points-cli
